...
all_filenames{1}='L7_FI_TheGB.uff'; selected_tx(1) = 64; tag{1} = 'FI';
all_filenames{2}='L7_DW_TheGB.uff'; selected_tx(2) = 1; tag{2} = 'DW';
all_filenames{3}='L7_CPWC_TheGB.uff'; selected_tx(3) = 1; tag{3} = 'PW';
all_filenames{4}='L7_STA_TheGB.uff'; selected_tx(4) = 32; tag{4} = 'STA';
...
for f = 1:length(all_filenames)
    ...
    b_data.plot(subplot(5,1,[2:5]),["Single Plane Wave Image"],[],[],[],[selected_tx(f)]);
    colorbar off


    %%
    b_data_illustrate_tx_delay = uff.beamformed_data(b_data);
    b_data_illustrate_tx_delay.data = mid.transmit_delay*10^6;
    b_data_illustrate_tx_delay.plot([],['Receive Transmit Delay'],[],['none']);
    title('Receive Transmit Delay');
    colormap jet;

    b_data_illustrate_rx_delay = uff.beamformed_data(b_data);
    b_data_illustrate_rx_delay.data = mid.receive_delay*10^6;
    b_data_illustrate_rx_delay.plot([],['Receive  Delay'],[],['none']);
    title('Receive  Delay');
    colormap jet;
...
fig = figure();
b_data_compare.plot(fig,['1=FI,2=PW,3=DW,4=STA']);
