...
s.firing_rate = 5000; % firing rate of output signal, (Doppler PRF) = (firing rate)/(nr of firings)
...
s.PSF_function = @PSFfunc_LinearProbe_DivergingWave;
...
s.PSF_params.acq.focal_depth     = -2e-2;
s.PSF_params.acq.apex            = -2e-2;  % virtual apex position [m], should be non-positive


s.PSF_params.scan.rxAngleSpanDeg = 10; 
s.PSF_params.scan.noRx = 50;
s.PSF_params.scan.zStart = 40e-3-s.PSF_params.acq.apex; %NB! Position is relative to apex
s.PSF_params.scan.zEnd = 60e-3-s.PSF_params.acq.apex; %NB! Position is relative to apex
s.PSF_params.scan.Nz = 128;

s.PSF_params.run.chunkSize = 121;
...
s.phantom_params.tubedepth = 0.05; %0.03;
